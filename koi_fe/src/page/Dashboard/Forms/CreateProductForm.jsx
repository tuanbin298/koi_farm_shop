...
export default function CreateProductForm({ setSelectedSection }) {
  ...
  const { data, refetch: refetchProducts } = useQuery(GET_ALL_PRODUCTS_ADMIN);
  ...
  const validateForm = () => {
    const newErrors = {};
    const priceRegex = /^\d+(\.\d{1,2})?$/; //  positive numbers with two decimals
    ...
    if (!productData.birth) {
      newErrors.birth = "Năm sinh không được để trống";
    }
    if (parseInt(productData.birth) < 0) {
      newErrors.birth = "Năm sinh không được âm";
    }
    ...
    if (!productData.price) {
      ...
      !priceRegex.test(productData.price) ||
      parseFloat(productData.price) <= 0
    ) {
      newErrors.price = "Giá không được âm";
    }
    if (!productData.origin) {
      newErrors.origin = "Nguồn cung không được để trống";
    }
    if (!productData.generic) {
      newErrors.generic = "Chủng loại không được để trống";
    }
    if (!productData.image) {
      newErrors.image = "Chưa có hình ảnh";
    }
    if (!productData.category) {
      newErrors.category = "Loại không được để trống";
    }
